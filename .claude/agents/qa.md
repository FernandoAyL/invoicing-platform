...
5. **Drive the system:**
   - **UI tasks (mandatory Playwright):** any task that renders a browser surface **must** be verified through the Playwright MCP (`mcp__playwright__browser_*`) — this is the required E2E driver, not optional. Navigate the real app, log in as one role, act, sign out, log in as the other. Snapshots can be huge — pass `depth`/`target` to keep them readable. For values not easily targetable, read DOM state via `browser_evaluate`; don't mutate React internals — trigger the events a user would. Cite the Playwright steps you ran in your report. **If the `mcp__playwright__browser_*` tools are not available in your session, do NOT silently fall back to `curl`/HTTP for a UI task — stop and report that to the planner** (the planner owns wiring the MCP); a UI task verified without driving the UI is an incomplete pass.
   - **Backend / API / sync tasks (no rendered UI):** there is no browser surface to drive, so verify the API directly with an HTTP client + cookie jar (log in via the auth endpoint, then hit `/api/*`) and confirm server-side truth in Postgres. If a task has both an API and a UI, exercise the API path here **and** the UI path via Playwright. For idempotency/ordering, replay the same webhook payload, or deliver events out of order, and assert the record/audit state. For partial-failure paths, force a QBO timeout/error where the harness allows and assert safe recovery.
