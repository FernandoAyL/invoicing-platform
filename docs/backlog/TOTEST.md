...
- ☐ `10002` CI (GitHub Actions) + Vitest setup.
  - **Done:** `.github/workflows/ci.yml` (on push to `main` + PRs): setup-node 24 → `corepack enable` → `pnpm install --frozen-lockfile` → `pnpm run ci` (Biome) → `pnpm -r typecheck` → `pnpm -r test` → `docker build`. Added Vitest (`apps/api` `test` script + root `test`, `apps/api/vitest.config.ts`); refactored `config.ts` to a testable `loadConfig(env)` and added `src/config.test.ts` (4 cases: valid env, defaults, missing `DATABASE_URL`, bad `PORT`). **Verified locally:** `vitest` 4/4 pass, `pnpm -r typecheck` and `biome ci` clean; every CI step run locally (docker build confirmed).
