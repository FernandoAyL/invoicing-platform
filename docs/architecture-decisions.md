...
One Firebase Hosting constraint this imposes: to keep its CDN cacheable, Hosting **strips every request cookie except one literally named `__session`** before proxying to the backend. So the session cookie must be named `__session` — any other name is set at login but silently dropped on the next request, and auth 401s. The cookie is named accordingly in `apps/api/src/plugins/auth.ts` (`SESSION_COOKIE`).
